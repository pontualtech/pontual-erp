...
async function processWebhook(cfg: BotCompanyConfig, body: any) {
  ...
  for (let loopIdx = 0; loopIdx < MAX_LOOPS; loopIdx++) {
    ...
    // ── BOT SUPORTE RULES (Marta/Aline): MOVIDAS pro pre_prompt (system) no Dify
    // em 03/09/2026 — otimizacao de custo de tokens. O bloco [REGRAS DA ...]
    // (~12.4k chars) era anexado a CADA query e re-acumulava no historico da
    // conversa (cada turno antigo carregava a propria copia — conversa de 4
    // turnos = 4 copias no input). No system prompt o modelo ve as MESMAS
    // regras 1x por chamada, sem duplicacao no historico. Validado por bateria
    // A/B (7 cenarios: tags, PIX exato, URLs — comportamento identico).
    // Backup dos prompts/blocos: .backup_dify_prompts_2026_09_03/ (workspace CURSO CLAUDE).
    // NAO mover o bloco [DADOS REAIS DA EMPRESA] acima pro system: a bateria v1
    // provou que longe da query o bot passa a OMITIR a chave PIX (recencia).
