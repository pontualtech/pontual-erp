...
export default function EmitirNfsePage() {
  ...
  const [osReference, setOsReference] = useState('')
  const [items, setItems] = useState<ServiceItem[]>([{ description: '', quantity: 1, unit_price_cents: 0 }])
  ...
  function updateItem(index: number, field: keyof ServiceItem, value: string | number) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setResult(null)
    try {
      const res = await fetch('/api/fiscal/emitir-nfse', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          customer_id: selectedCustomerId,
          os_reference: osReference || undefined,
          items: items.map(i => ({
      ...
      if (res.ok) {
        setResult({ success: true, message: data.message ?? 'NFS-e enviada para processamento!' })
      } else {
        setResult({ success: false, message: data.error ?? 'Erro ao emitir NFS-e' })
      }
    } catch {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-yellow-300 bg-yellow-50 px-4 py-3 text-sm text-yellow-800">
        <strong>HOMOLOGACAO</strong> — Esta emissao sera feita em ambiente de homologacao (testes). Nenhuma nota fiscal de servico real sera gerada.
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">OS Referencia (opcional)</label>
            <input
              placeholder="Numero da OS relacionada"
              value={osReference}
              onChange={e => setOsReference(e.target.value)}
            ...
            {items.map((item, idx) => (
              <div key={idx} className="flex items-end gap-3">
                <div className="flex-1">
                  {idx === 0 && <span className="text-xs text-gray-500">Descricao do servico</span>}
                  <input
                    required
