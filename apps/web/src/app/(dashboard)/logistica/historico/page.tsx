...
import {
  ArrowLeft, Loader2, Truck, CheckCircle2, XCircle, Clock,
...
function TrendChart({ data }: { data: Metrics['trend'] }) {
  ...
  if (data.length === 0) {
    return <p className="text-sm text-gray-500">Sem dados no periodo.</p>
  }
  return (
...
export default function HistoricoPage() {
  ...
  const applyPreset = useCallback((key: PresetKey) => {
    ...
    if (key === 'today') {
      ...
      setFrom(daysAgoISO(p.fromDays))
      setTo(todayISO())
    }
    setPage(1)
  }, [])

  // Carrega lista de motoristas (uma vez)
  useEffect(() => {
    let cancelled = false
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Dashboard e listagem completa de todas as rotas e paradas.</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex-1 max-w-xs ml-4">
            <input
              placeholder="Buscar por motorista ou nota..."
              value={search}
                  ...
                  return (
                    <tr key={r.id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-2 px-2 text-right">
                        ...
                        )}>{r.total_stops > 0 ? `${r.success_rate}%` : '—'}</span>
                      </td>
